...
function AESCipher(key, mode) {
  ...
  this.cipher = new AES.ModeOfOperation[mode.toLowerCase()](raw.decode(key));
}
...
AESCipher.prototype.encrypt = function(bytes) {
  return this.cipher.encrypt(raw.decode(bytes));
};
...
AESCipher.prototype.decrypt = function(encrypted) {
  return this.cipher.decrypt(raw.decode(encrypted));
};
