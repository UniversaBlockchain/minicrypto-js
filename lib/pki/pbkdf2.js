...
const { bytesToArray, arrayToBytes } = utils;
...
function derive(hash, options, callback) {
  ...
  const convertedSalt = typeof salt == 'object' ? arrayToBytes(salt) : salt;

  if (!callback)
    return bytesToArray(forge.pbkdf2(password, convertedSalt, iterations, keyLength, forgeMD));

  forge.pbkdf2(password, convertedSalt, iterations, keyLength, forgeMD, (err, key) => {
