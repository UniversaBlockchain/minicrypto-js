...
function derive(hash, options, callback) {
  ...
  const convertedSalt = typeof salt == 'object' ? arrayToByteString(salt) : salt;

  if (!callback)
    return byteStringToArray(forge.pbkdf2(password, convertedSalt, iterations, keyLength, forgeMD));
