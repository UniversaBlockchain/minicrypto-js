...
module.exports = class PublicKey {
  ...
  encrypt(bytes, options) {
    return this.key.encrypt(arrayToBytes(bytes), 'RSA-OAEP', wrapOptions(options));
  }
  ...
  address(options = {}) {
    ...
    const concat = new Uint8Array(modulus.length + exponent.length);
    concat.set(exponent, 0);
    concat.set(modulus, exponent.length);
    // hash.put(exponent);
    // hash.put(modulus);
    const hashed = hash.get(concat);
