...
const { decode64, encode64, BigInteger, itob, btoi, bytesToHex, hexToBytes, raw, byteStringToBin } = utils;
const { pki } = forge;
const { rsa } = pki;
...
module.exports = class PublicKey {
  ...
  fingerprint() {
    ...
    const hashedExponents = sha256.get('bytes');

    this._fingerprint = boss.pack(0, byteStringToBin(hashedExponents));
...
function fromBOSS(dump) {
  const boss = new Boss();
  const parts = boss.load(raw.encode(decode64(dump)));
