...
function toBOSS(key) {
  ...
  return encode64(raw.decode(boss.dump([
    ...
    hexToBytes(p.toString(16)),
    hexToBytes(q.toString(16))
  ])));
}
...
function fromBOSS(dump) {
  ...
  return fromExponents({
    e: new BigInteger(bytesToHex(parts[1]), 16),
    p: new BigInteger(bytesToHex(parts[2]), 16),
    q: new BigInteger(bytesToHex(parts[3]), 16)
  });
}
