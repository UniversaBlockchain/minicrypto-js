...
exports.sign = (key, data) => {
	...
	const targetSignature = boss.dump({
		'key': keyId(key),
		'sha512': bytesToArray(dataHash.get(data)),
		'created_at': new Date()
	});

	return boss.dump({
		'exts': targetSignature,
		'sign': bytesToArray(key.sign(arrayToBytes(targetSignature), { pssHash: new SHA(512), mgf1Hash: new SHA(1) }))
	});
}
...
exports.verify = (publicKey, signature, data) => {
	...
	const verified = publicKey.verify(arrayToBytes(exts), arrayToBytes(sign), { pssHash: new SHA(512), mgf1Hash: new SHA(1) });
	...
	if (dataHash.get(data) === arrayToBytes(sha512)) return { key, created_at };
