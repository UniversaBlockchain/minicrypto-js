...
describe('Signed record', function() {
  ...
  it('should sign with key without nonce', () => {
    const key = PrivateKey.unpack(privateKeyPacked);
    const record = SignedRecord.packWithKey(key, { ab: "cd" });
  ...
  it('should sign with key with nonce', () => {
    const key = PrivateKey.unpack(privateKeyPacked);
    const nonce = decode64("abcd");
    const record = SignedRecord.packWithKey(key, { ab: "cd" }, nonce);
    const recordObj = boss.load(record);
    expect(recordObj[0]).to.equal(0);
  ...
  it('should unpack', () => {
    const key = PrivateKey.unpack(privateKeyPacked);
    const nonce = decode64("abcd");
    const recordPacked = SignedRecord.packWithKey(key, { ab: "cd" }, nonce);

    const record = SignedRecord.unpack(recordPacked);
