...
const { bytesToHex, hexToBytes, textToHex, raw, decode64, bytesToArray } = utils;
...
describe('Extended signature', function() {
  it('should get key id', function() {
    const key = new PrivateKey('BOSS', readKey64(vectors.keys[1]));
    const id = keyId(key);

    should(bytesToHex(id)).eql('074118648ed82a64b9a9ff6a9cb7bcd64cf5367e290e1c80c333a08107c1f82663');
  });

  it('should sign and verify data', function() {
    const data = bytesToArray(hexToBytes(textToHex('Hello world')));
    const key = new PrivateKey('BOSS', readKey64(vectors.keys[3]));
    const id = keyId(key);
